...
// Donut chart color palette — soft, premium tones
const CHART_COLORS = [
  "#34d399", // mint green
  "#60a5fa", // soft blue
  "#a78bfa", // violet
...
export default function StrategyTab() {
  const [targets, setTargets] = useState<TargetRow[]>([
    { id: crypto.randomUUID(), asset_ticker: "", target_percentage: 0 },
  ]);
  ...
  const totalPercentage = useMemo(
    () => targets.reduce((sum, t) => sum + (t.target_percentage || 0), 0),
    [targets]
  );
  ...
  const chartData = useMemo(() => {
    ...
    if (remaining > 0) {
      items.push({ name: "Unallocated", value: remaining });
    }
  ...
  return (
    <div className="tab-content-enter">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2
            style={{ color: "var(--text-primary)", fontSize: 24, fontWeight: 700 }}
          >
            The Strategy
          </h2>
          <p style={{ color: "var(--text-secondary)", fontSize: 14, marginTop: 4 }}>
            Define your portfolio recipe. Set a target allocation for each asset.
          </p>
        </div>
      </div>

      {/* Main Layout: Table + Chart side by side */}
        ...
        <div>
          ...
          <div className="card-elevated" style={{ padding: "16px 20px", marginBottom: 24 }}>
            ...
            {isOver && (
              ...
              >
                <AlertCircle size={14} />
                Total exceeds 100% by {(totalPercentage - 100).toFixed(1)}%
              </div>
            )}
            {isValid && (
              <div
          ...
          <div className="card" style={{ overflow: "hidden" }}>
            ...
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "1fr 140px 56px",
                gap: 12,
                padding: "12px 20px",
            ...
            {targets.map((target, index) => (
              ...
              >
                <input
                  type="text"
                  placeholder="e.g. VOO, BTC-USD"
                  value={target.asset_ticker}
                  onChange={(e) =>
                    updateRow(target.id, "asset_ticker", e.target.value.toUpperCase())
                  }
                  style={{
                    background: "var(--bg-secondary)",
                ...
                <button
                  ...
                  style={{
                    ...
                    transition: "background 0.15s",
                    opacity: targets.length <= 1 ? 0.3 : 1,
                  }}
                  onMouseEnter={(e) => {
                    if (targets.length > 1)
          ...
          >
            <button className="btn-ghost" onClick={addRow} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <Plus size={16} />
              Add Asset
            </button>
            <button
